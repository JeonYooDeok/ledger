...
function SignUp() {
  ...
  const handleValidation = (field, value) => {
    ...
    switch (field) {
      ...
      case "passwordConfirm":
        errorMsg =
          value.trim() === "" // 입력 값이 비어있는지 확인
            ? "비밀번호를 확인해주세요." // 입력 값이 비어있을 때 에러메시지
            : values.password === value // 정규식 검사
            ? "" // 비밀번호와 비밀번호 확인이 일치하는 경우
            : "비밀번호가 일치하지 않습니다."; // 비밀번호와 비밀번호 확인이 일치하지 않는 경우
        break;
      default:
        break;
    }
  ...
  const signUpHandler = async (e) => {
    ...
    handleValidation("passwordConfirm", values.passwordConfirm);
    ...
    try {
      // 실제 API 호출 (현재는 주석 처리됨)
      // const response = await axios.post("/public-api/v1/member/join", {
      //   email: values.id,
      //   password: values.password,
      //   // 필요한 추가 데이터
      // });
      // navigate("/signupcomplete"); // 성공 시 회원가입 완료 페이지로 이동
      ...
      setIsLoading(false); // 회원가입 프로세스 종료, 로딩 상태 비활성화
    }
  };
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-start gap-2">
              <TextBox
                ...
                required
              />
              <SecondaryButton label="인증번호 전송" className="w-40 mt-6" />
            </div>
            <div className="flex items-start gap-2">
              <TextBox
                label="인증번호"
                ...
                onChange={handleInputChange}
              />
              <SecondaryButton label="인증번호 확인" className="w-40 mt-6" />
            </div>
            <TextBox
              label="비밀번호"
              placeholder="비밀번호 입력"
            ...
            <TextBox
              ...
              type="password"
              errorMessage={message.passwordConfirm}
              onChange={handleInputChange}
              className="w-full"
            />
            <PrimaryButton
